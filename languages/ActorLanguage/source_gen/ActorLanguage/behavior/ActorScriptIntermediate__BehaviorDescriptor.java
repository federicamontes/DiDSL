...
public final class ActorScriptIntermediate__BehaviorDescriptor extends BaseBHDescriptor {
  private static final SAbstractConcept CONCEPT = MetaAdapterFactory.getConcept(0x10eda99958984cdeL, 0x9416196c5eca1268L, 0x46a144cd5bb2ddL, "ActorLanguage.structure.ActorScriptIntermediate");
  ...
  /*package*/ static Iterable<SNode> allReferenceableContentsInChunk_id6clJcrKmVSn(@NotNull SNode __thisNode__) {
    ...
    ListSequence.fromList(added).addSequence(Sequence.fromIterable(SLinkOperations.collect(SNodeOperations.ofConcept(SLinkOperations.getChildren(__thisNode__, LINKS.types$SV_k), CONCEPTS.ExternalType$Bi), LINKS.structDeclaration$Nldq)));
    ListSequence.fromList(added).addSequence(Sequence.fromIterable(SNodeOperations.ofConcept(SLinkOperations.getChildren(__thisNode__, LINKS.types$SV_k), CONCEPTS.CustomType$VF)));
    ListSequence.fromList(added).addSequence(Sequence.fromIterable(SNodeOperations.ofConcept(SLinkOperations.getChildren(__thisNode__, LINKS.types$SV_k), CONCEPTS.ExternalTypeDefinition$1k)));
    ListSequence.fromList(added).addSequence(Sequence.fromIterable(SLinkOperations.collect(SNodeOperations.ofConcept(SLinkOperations.getChildren(__thisNode__, LINKS.types$SV_k), CONCEPTS.OpaqueTypeDeclaration$jq), LINKS.opaqueType$mnjr)));
    ListSequence.fromList(added).addSequence(Sequence.fromIterable(SNodeOperations.ofConcept(SLinkOperations.getChildren(__thisNode__, LINKS.externalFunctions$SVmj), CONCEPTS.ExternalFunction$kh)));
    ListSequence.fromList(added).addSequence(Sequence.fromIterable(SLinkOperations.collect(SNodeOperations.ofConcept(SLinkOperations.getChildren(__thisNode__, LINKS.externalFunctions$SVmj), CONCEPTS.ExternalFunctionPrototype$n3), LINKS.prototype$75Zb)));
    ListSequence.fromList(added).addSequence(Sequence.fromIterable(SNodeOperations.ofConcept(SLinkOperations.getChildren(__thisNode__, LINKS.configuration$SUbe), CONCEPTS.GlobalVarDecl$3_)));
    ListSequence.fromList(added).addSequence(Sequence.fromIterable(SNodeOperations.ofConcept(SLinkOperations.getChildren(__thisNode__, LINKS.configuration$SUbe), CONCEPTS.GlobalConstant$pG)));
    ListSequence.fromList(base).addSequence(ListSequence.fromList(added));
    return base;
  }

  /*package*/ ActorScriptIntermediate__BehaviorDescriptor() {
  }
